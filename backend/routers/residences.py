...
@router.post("/{residence_id}/photos")
async def ajouter_photo(
    ...
    if res.proprietaire_id != current_user.id and current_user.role != "admin":
        raise HTTPException(403, "Non autorisé")
    url = save_file(photo, "photos")
    photos = res.photos_supplementaires or []
    photos.append(url)
    res.photos_supplementaires = photos
    db.commit(); db.refresh(res)
    return {"photos": res.photos_supplementaires}

@router.delete("/{residence_id}/photos")
async def supprimer_photo(
    ...
    if res.proprietaire_id != current_user.id and current_user.role != "admin":
        raise HTTPException(403, "Non autorisé")
    photos = [p for p in (res.photos_supplementaires or []) if p != photo_url]
    res.photos_supplementaires = photos
    db.commit()
    return {"photos": photos}

@router.put("/{residence_id}/equipements")
async def modifier_equipements(
    residence_id: int,
    ...
    if res.proprietaire_id != current_user.id and current_user.role != "admin":
        raise HTTPException(403, "Non autorisé")
    try: equip_list = json.loads(equipements)
    except: equip_list = []
    res.equipements = equip_list
    db.commit(); db.refresh(res)
