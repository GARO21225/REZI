...
def get_reservations_proprietaire(
    current_user: User = Depends(get_current_active_user),
    db: Session = Depends(get_db)
):
    # Trouver les résidences du propriétaire
    from models.residence import Residence as Res
    from models.user import User as U
    mes_residences = db.query(Res).filter(Res.proprietaire_id == current_user.id).all()
    ids = [r.id for r in mes_residences]
    if not ids: return []
    
    reservations = db.query(Reservation).filter(
        Reservation.residence_id.in_(ids)
    ).order_by(Reservation.created_at.desc()).all()
    
    result = []
    for r in reservations:
        res = next((x for x in mes_residences if x.id == r.residence_id), None)
        usager = db.query(U).filter(U.id == r.usager_id).first()
        d = resa_to_dict(r)
        d['residence_titre'] = res.titre if res else ''
        d['usager_nom'] = f"{usager.prenom} {usager.nom}" if usager else ''
        d['nb_nuits'] = max(1, (r.date_fin - r.date_debut).days) if r.date_fin and r.date_debut else 1
        result.append(d)
    return result
